...
export default function AdminScreen() {
  ...
  const confirmDeleteUser = (person: any) => {
    if (deletingUser || person.user_id === user?.user_id || person.email?.toLowerCase() === "utkarsh7023340530@gmail.com") return;
    Alert.alert(
      "Delete this account?",
      `This permanently removes ${person.name || person.email || "this user"}'s login, profile and private data, signs them out everywhere, and removes their active trips. Shared integrity records may keep an anonymized Deleted user reference. This cannot be undone.`,
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          style: "destructive",
          onPress: async () => {
            setDeletingUser(person.user_id);
            try {
              await api.adminDeleteUser(person.user_id);
              await load();
              Alert.alert("Account deleted", `${person.name || person.email || "The user"} has been removed from UniPool.`);
            } catch (e: any) {
  ...
  return <SafeAreaView style={styles.safe} edges={["top"]}><ScrollView contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled">
    ...
    {!moderatorOnly ? <><View style={styles.sectionHead}><View><Text style={styles.eyebrow}>DIRECTORY</Text><Text style={styles.sectionTitle}>{people.length} registered people</Text></View><Pressable onPress={load}><Ionicons name="refresh" size={20} color={colors.indigo} /></Pressable></View>
    ...
    {loading ? <ActivityIndicator color={colors.indigo} style={{ marginTop: 40 }} /> : filtered.map((person) => <View key={person.user_id} style={styles.person}><Pressable onPress={() => router.push({ pathname: "/network", params: { userId: person.user_id, name: person.name || "Traveller" } } as any)} style={{ flex: 1, flexDirection: "row", alignItems: "center", gap: 11 }}><View style={styles.avatar}><Text style={styles.avatarText}>{String(person.name || "S").slice(0, 1).toUpperCase()}</Text></View><View style={{ flex: 1 }}><Text style={styles.name}>{person.name || person.username || "Traveller"}</Text><Text style={styles.meta}>{person.email || "No email"}{person.college_verified ? " · MU verified" : ""}</Text>{isOwner ? <Text style={styles.meta}>Role: {person.role || "user"}</Text> : null}</View></Pressable><View style={styles.personActions}>
      {isOwner && person.email?.toLowerCase() !== "utkarsh7023340530@gmail.com" ? <Pressable disabled={Boolean(changingRole) || Boolean(deletingUser)} onPress={() => chooseRole(person)} style={styles.retry}><Text style={styles.retryText}>{changingRole === person.user_id ? "Saving..." : "Change role"}</Text></Pressable> : null}
      {person.user_id !== user?.user_id && person.email?.toLowerCase() !== "utkarsh7023340530@gmail.com" ? <Pressable accessibilityLabel={`Delete account for ${person.name || person.email || "user"}`} disabled={Boolean(deletingUser)} onPress={() => confirmDeleteUser(person)} style={styles.deleteUserBtn}>{deletingUser === person.user_id ? <ActivityIndicator size="small" color={colors.error} /> : <Ionicons name="trash-outline" size={18} color={colors.error} />}</Pressable> : null}
    </View></View>)}
    {!loading && !filtered.length ? <Text style={styles.empty}>No people match this filter.</Text> : null}</> : null}
