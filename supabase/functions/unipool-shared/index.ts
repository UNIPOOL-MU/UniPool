...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (path === "/messages/conversations" && req.method === "GET") {
      ...
      for(const c of convos||[]){const {data:last}=await db.from("unipool_messages").select("*").eq("conversation_id",c.id).order("created_at",{ascending:false}).limit(1).maybeSingle(); if(c.kind==="direct"){const parts=String(c.direct_key).split("::");const other=parts[0]===user.user_id?parts[1]:parts[0];const names=await directoryMap([other]);const {count:unread}=await db.from("unipool_messages").select("id",{count:"exact",head:true}).eq("conversation_id",c.id).eq("to_user_id",user.user_id).is("read_at",null);const {data:p}=await db.from("unipool_presence").select("last_seen_at").eq("user_id",other).maybeSingle();result.push({kind:"direct",other_user_id:other,conversation_id:c.id,name:names.get(other)?.name||"Student",picture:names.get(other)?.picture||null,last_message:last?.text||"Start a conversation",last_message_from_user_id:last?.from_user_id||null,last_at:last?.created_at||c.updated_at,unread:unread||0,online:p?Date.now()-new Date(p.last_seen_at).getTime()<60000:false});}else{const {count}=await db.from("unipool_conversation_members").select("user_id",{count:"exact",head:true}).eq("conversation_id",c.id);result.push({kind:"group",conversation_id:c.id,name:c.name||"Trip chat",last_message:last?.text||"Trip chat ready",last_message_from_user_id:last?.from_user_id||null,last_at:last?.created_at||c.updated_at,unread:0,members_count:count||1});}}
    ...
    const ensureMatch=path.match(/^\/messages\/trip\/ensure\/([^/]+)$/);if(ensureMatch&&req.method==="POST"){const poolId=ensureMatch[1];let {data:c}=await db.from("unipool_conversations").select("*").eq("pool_id",poolId).maybeSingle();if(!c){const rp=await renderFetch(`/api/pools/${encodeURIComponent(poolId)}`,user.token);if(!rp?.ok)return fail("Trip details are temporarily unavailable",503);const pool=await rp.json();const confirmed=(pool.confirmed_travelers||[]).map((t:any)=>t.user_id);const allowed=pool.user_id===user.user_id||confirmed.includes(user.user_id);if(!allowed)return fail("Join the trip before opening its group chat",403);const name=`${pool.from_location||"From"} → ${pool.to_location||"To"}`;const ins=await db.from("unipool_conversations").insert({kind:"group",pool_id:poolId,name,legacy_conversation_id:pool.trip_conversation_id||null,updated_at:nowIso()}).select("*").single();c=ins.data;const members=[pool.user_id,...confirmed,user.user_id].filter(Boolean);await db.from("unipool_conversation_members").upsert([...new Set(members)].map((id)=>({conversation_id:c.id,user_id:id})),{onConflict:"conversation_id,user_id"});for(const t of pool.confirmed_travelers||[])if(t.user_id&&t.name)await db.from("unipool_user_directory").upsert({user_id:t.user_id,name:t.name,email:t.email||null,updated_at:nowIso()});if(pool.user_id&&pool.user_name)await db.from("unipool_user_directory").upsert({user_id:pool.user_id,name:pool.user_name,email:pool.user_email||null,updated_at:nowIso()});}const {data:member}=await db.from("unipool_conversation_members").select("user_id").eq("conversation_id",c.id).eq("user_id",user.user_id).maybeSingle();if(!member)return fail("You're not in this trip chat",403);return json({conversation_id:c.id,name:c.name||"Trip chat"});}
    const groupMatch2=path.match(/^\/messages\/group\/([^/]+)$/);if(groupMatch2&&req.method==="GET"){const cid=groupMatch2[1];const {data:member}=await db.from("unipool_conversation_members").select("*").eq("conversation_id",cid).eq("user_id",user.user_id).maybeSingle();if(!member)return fail("Trip chat not found",404);const {data:c}=await db.from("unipool_conversations").select("*").eq("id",cid).single();const {data:members}=await db.from("unipool_conversation_members").select("*").eq("conversation_id",cid);const ids=(members||[]).map((m:any)=>m.user_id);const names=await directoryMap(ids);const readMap=new Map((members||[]).map((m:any)=>[m.user_id,m.last_read_at]));if(c.legacy_conversation_id){const {count:imported}=await db.from("unipool_messages").select("id",{count:"exact",head:true}).eq("conversation_id",cid).like("legacy_message_id","group-import:%");if(!imported){const previous=await renderFetch("/api/messages/group/"+encodeURIComponent(c.legacy_conversation_id),user.token);if(previous?.ok){const thread=await previous.json();for(const m of thread.messages||[]){if(!m.message_id||!ids.includes(m.from_user_id))continue;await db.from("unipool_messages").upsert({legacy_message_id:"group-import:"+m.message_id,conversation_id:cid,from_user_id:m.from_user_id,text:String(m.text||""),created_at:safeDate(m.created_at).toISOString()},{onConflict:"legacy_message_id",ignoreDuplicates:true});}}}}const {data:msgs}=await db.from("unipool_messages").select("*").eq("conversation_id",cid).order("created_at",{ascending:true}).limit(1000);await db.from("unipool_conversation_members").update({last_read_at:nowIso()}).eq("conversation_id",cid).eq("user_id",user.user_id);return json({conversation_id:cid,name:c.name||"Trip chat",members:ids.map((id:string)=>({user_id:id,name:names.get(id)?.name||"Student"})),messages:(msgs||[]).map((m:any)=>({message_id:m.legacy_message_id||m.id,from_user_id:m.from_user_id,text:m.text,created_at:m.created_at,read_by:ids.filter((id:string)=>{const t=readMap.get(id);return t&&new Date(t).getTime()>=new Date(m.created_at).getTime();})}))});}
