...
## Message alerts, signup tour and header

New non-imported Supabase messages now create notifications for the recipient of a direct chat or all other group members. Updating a member's read timestamp clears that chat's notification records. A transaction that rolled back verified direct/group recipients, sender exclusion, read clearing and historical-import exclusion. Both direct and group chat use the shared message service; trip chat entry points resolve the shared conversation ID and can import prior group history without generating new-message alerts.

The header bell and message banner share the people-service inbox. A five-second poll detects new messages and opens their chat from the banner. With browser notification permission granted, an active site can also show a device notification via its service worker. This change does not establish or verify closed-site Web Push delivery: that still depends on the existing push subscription backend, VAPID configuration and device permission. A failed push enable request no longer saves an enabled preference.

Tour eligibility comes from explicit signup flows (or a newly created OAuth account). Local completion wins even if later auth responses omit the field; account completion is stored by the people service. Normal returning logins do not show the signup tour. Header Explore was removed.

The updated general browser regression pass completed 75 checks with no page errors. The focused `scripts/qa/notifications-tour.mjs` pass covers skip persistence, omitted completion fields, returning accounts, header removal, new message banner, unread bell and chat navigation. Source inventory is now 375 controls across 56 screens/components. TypeScript and Expo export passed.
